...
- **Local CLI for bots.** Jobs get `COGA_SERVE_CMD=<repo>/node_modules/.bin/tsx packages/cli/src/index.ts` unless already set. The harness default (`npx -y coordination-games@latest`) cold-downloads and can miss the MCP startup window → bots come up with zero coga tools and burn their session (observed live; transcript shows the model complaining it has no coga tools, `reason: cap` after 1 call).
