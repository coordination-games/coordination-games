...
- `get_guide()` for game rules instead of hardcoded prompts
...
- Connect to MCP, call `get_state()`, try random valid moves until one succeeds
...
- Claude Haiku bots that read `get_guide()` and play intelligently
