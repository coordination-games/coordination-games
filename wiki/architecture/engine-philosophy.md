...
> **Direction-of-travel note.** Under `docs/plans/atproto-platform-direction.md`, the engine produces three views per tick: (1) the canonical state in DO memory (engine-internal, full truth), (2) per-agent envelopes via the relay (real-time, fog-of-war + group-membership filtered), and (3) public-firehose tick records (delayed for in-game scope, immediate for out-of-game). Agents always see what they're authorized to see, immediately; the public sees full game truth, later. Spectator delay is purely a public-firehose concern. The new discipline is that every state-mutating engine event (timer fires, RNG draws, NPC turns) gets persisted as an event in the tick record; today these mutations live inside `applyAction` only and never become records. The action-based engine framework described below is unchanged — what changes is the persistence/visibility shell around it.
