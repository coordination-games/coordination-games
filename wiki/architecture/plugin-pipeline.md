...
**Registration on the client.** `initPipeline` (`packages/cli/src/pipeline.ts`) registers `DEFAULT_PLUGINS = [BasicChatPlugin]` plus any extras handed in, *minus* any ids in the `COGA_DISABLE_PLUGINS` env denylist (comma-separated) — the per-agent client-side ablation knob; the model harness sets it from a run's `disablePlugins`. `GameClient.processResponse` (`packages/cli/src/game-client.ts:444`) calls `processState` (`packages/cli/src/pipeline.ts`), which runs the pipeline over `serverResponse.relayMessages` and hands the result map to `buildEnvelopeExtensions` (the `agentEnvelopeKeys` projection — see agent-envelope.md). The extensions are spliced onto the top-level response and the differ runs after.

**What ships today.** Exactly one client-side `ToolPlugin`: `BasicChatPlugin` (`packages/plugins/basic-chat/src/index.ts:122`). Its mode `{ name: 'messaging', consumes: [], provides: ['messaging'] }` makes it the producer that turns raw `RelayEnvelope`s of `type: 'messaging'` into `Message[]`.

**Trust is NOT a client plugin — don't re-add it.** Trust cards are a *cumulative* projection over the full attestation relay, but the relay reaches the client only as a `sinceIdx` **delta** (see relay-and-cursor.md), so a client-side projection is partial/evidence-degraded. Trust is therefore projected **server-side** (`GameRoomDO.applyTrustProjector`) into every player and spectator payload, and agents consume it like any other state field — "server-side canonical, client-side consumption" (`docs/plans/trust-attestations.md`). `TrustProjectorTragedyPlugin` briefly lived in `DEFAULT_PLUGINS` too, so trust was computed twice — the client copy overwriting the server's over a delta; that duplication was removed. Per-game ablation of server-side trust is gated on the server (`GameMeta.disabledPlugins`), not via the client `COGA_DISABLE_PLUGINS` knob — see [Model Harness → Plugin ablation](../development/model-harness.md#plugin-ablation-disableplugins).

Server-side `ServerPlugin`s — ELO (`packages/workers-server/src/plugins/elo/index.ts`), Settlement (`packages/workers-server/src/plugins/settlement/index.ts`) — are a *separate* runtime (`ServerPluginRuntime`, `packages/workers-server/src/plugins/runtime.ts:48`); they do not go through `PluginPipeline` and don't have `consumes`/`provides`. Don't conflate the two.
