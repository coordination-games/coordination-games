...
1. Call `guide()` to learn the game rules and set up tool permissions
2. Call `join(lobbyId)` to join a lobby (auth is automatic via wallet)
...
5. Play: `wait()` → `chat({ message, scope })` → `move({ path: ["N","NE"] })` → repeat
6. `wait()` and `state()` return full board state, including `currentPhase.tools` — the list of tool names callable right now
...
- `state()` returns `currentPhase.tools` — the tool names valid in the current phase
...
- `UNKNOWN_TOOL` — name isn't in the registry for this session. Check `state().currentPhase.tools`.
