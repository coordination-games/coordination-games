import type { Commitment, TrustCard, TrustSignal, VisibleBehaviorTag } from '../store';
...
function signalWeight(signal: TrustSignal): number {
  ...
  const label = `${signal.label} ${signal.summary}`.toLowerCase();
  let stance = 0;
  if (signal.stance === 'positive') stance = 0.72;
  else if (signal.stance === 'negative') stance = -0.72;

  if (label.includes('extract')) stance -= 0.18;
  if (label.includes('build')) stance += 0.18;
  if (label.includes('trade')) stance += 0.06;

  return clamp(stance * confidenceWeight, -1, 1);
}

function trustCardEvidenceCount(card: TrustCard): number {
  const refs = new Set<string>();
  for (const ref of card.evidenceRefs) refs.add(`${ref.kind}:${ref.id}`);
  for (const signal of card.signals) {
    for (const ref of signal.evidenceRefs ?? []) refs.add(`${ref.kind}:${ref.id}`);
  }
  return refs.size;
}

function typologyFromTrustCard(card: TrustCard | null): TypologyResult {
  if (!card) {
    return {
      row: bucketFromScore(0),
      col: bucketFromScore(0),
      cooperation: 'Neutral',
      reliability: 'Steady',
      cooperationScore: 0,
      reliabilityScore: 0,
      reasons: {
        cooperation: ['No primitive-backed trust card emitted yet'],
        reliability: ['Waiting for visible evidence references'],
      },
      stats: {
        fulfilledCount: 0,
        breachedCount: 0,
  ...
  const signalScores = card.signals.map(signalWeight);
  const cooperationScore =
    signalScores.length > 0
      ? clamp(signalScores.reduce((total, value) => total + value, 0) / signalScores.length, -1, 1)
      : 0;

  const evidenceCount = trustCardEvidenceCount(card);
  const confidenceValues = card.signals
  ...
  const meanConfidence =
    confidenceValues.length > 0
      ...
      : 0.5;
  const reliabilityScore = clamp(-0.15 + evidenceCount * 0.18 + meanConfidence * 0.34, -1, 1);

  const row = bucketFromScore(cooperationScore);
  const col = bucketFromScore(reliabilityScore);
  const positiveSignals = card.signals.filter((signal) => signal.stance === 'positive').length;
  const negativeSignals = card.signals.filter((signal) => signal.stance === 'negative').length;
  ...
  return {
    ...
    reasons: {
      cooperation: [
        `${card.signals.length} primitive trust-card signal${card.signals.length === 1 ? '' : 's'}`,
        card.headline,
      ],
      reliability: [
        `${evidenceCount} bounded evidence reference${evidenceCount === 1 ? '' : 's'}`,
        `Mean signal confidence ${meanConfidence.toFixed(2)}`,
      ],
    },
    stats: {
      fulfilledCount: positiveSignals,
      breachedCount: negativeSignals,
      pendingCount: Math.max(0, card.signals.length - positiveSignals - negativeSignals),
      totalCommitments: card.signals.length,
      completionRatio: meanConfidence,
      peerMean: cooperationScore / 2 + 0.5,
      peerVariance: 0,
      positiveTags: positiveSignals,
      negativeTags: negativeSignals,
    },
  };
}

export function computeTypology(ctx: TypologyContext): TypologyResult {
  if ('trustCard' in ctx) return typologyFromTrustCard(ctx.trustCard ?? null);
