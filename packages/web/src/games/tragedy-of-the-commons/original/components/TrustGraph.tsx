...
export function TrustGraph() {
  ...
  const displayAgents = useMemo(() => {
    ...
    if (trustCards.length > 0) return trustCards.map((card) => card.agentId);
    if (trustMatrix?.agents?.length) return trustMatrix.agents;
    return Object.keys(agentsMap);
  }, [agentOrder, trustCards, trustMatrix, agentsMap]);
  ...
  const positions = useMemo(() => {
    ...
    for (const id of displayAgents) {
      const trustCard =
        trustCards.find((card) => card.agentId === id || card.subjectId === id) ?? null;
    ...
    return out;
  }, [displayAgents, trustCards, agentsMap, trustMatrix, commitments, behaviorTags]);

  const activePosition = selectedId ? positions[selectedId] : null;
  const activeTrustCard = useMemo(
    () =>
      trustCards.find((card) => card.agentId === selectedId || card.subjectId === selectedId) ??
      null,
