...
// .wrangler/tmp/bundle-zAZCke/checked-fetch.js
...
function hashPair(a, b2) {
  const sorted = [a, b2].sort();
  return hashString(sorted[0] + sorted[1]);
}
__name(hashPair, "hashPair");
function encodeLeaf(data) {
  const payload = [
...
__name(buildMerkleTree, "buildMerkleTree");
function buildActionMerkleTree(actions) {
  const leaves = actions.map((action) => encodeLeaf(action));
  return buildMerkleTree(leaves);
}
__name(buildActionMerkleTree, "buildActionMerkleTree");

// ../engine/dist/registry.js
...
__name(getRegisteredGames, "getRegisteredGames");

// ../engine/dist/phases/open-queue.js
var OpenQueuePhase = class {
  static {
    __name(this, "OpenQueuePhase");
  }
  minPlayers;
  id = "open-queue";
  name = "Open Queue";
  acceptsJoins = true;
  timeout = null;
  constructor(minPlayers = 4) {
    this.minPlayers = minPlayers;
  }
  init(players) {
    return { playerIds: players.map((p) => p.id) };
  }
  handleAction(state, _action, _players) {
...
// ../engine/dist/server/auth.js
import crypto3 from "node:crypto";
var DEFAULT_CHALLENGE_TTL_MS = 5 * 60 * 1e3;
var DEFAULT_SESSION_TTL_MS = 24 * 60 * 60 * 1e3;

// ../engine/dist/server/framework.js
import crypto4 from "node:crypto";

// ../games/capture-the-lobster/dist/hex.js
var DIRECTIONS = {
  N: { q: 0, r: -1 },
  S: { q: 0, r: 1 },
  NE: { q: 1, r: -1 },
...
var ALL_DIRECTIONS = ["N", "NE", "SE", "S", "SW", "NW"];
function hexAdd(a, b2) {
  return { q: a.q + b2.q, r: a.r + b2.r };
}
__name(hexAdd, "hexAdd");
function hexEquals(a, b2) {
  return a.q === b2.q && a.r === b2.r;
}
__name(hexEquals, "hexEquals");
function getNeighbor(hex, direction) {
  return hexAdd(hex, DIRECTIONS[direction]);
}
__name(getNeighbor, "getNeighbor");
function getNeighbors(hex) {
  return ALL_DIRECTIONS.map((d) => getNeighbor(hex, d));
}
__name(getNeighbors, "getNeighbors");
function hexDistance(a, b2) {
  const dq = a.q - b2.q;
  const dr = a.r - b2.r;
  return (Math.abs(dq) + Math.abs(dr) + Math.abs(dq + dr)) / 2;
}
__name(hexDistance, "hexDistance");
...
__name(hexesInRadius, "hexesInRadius");
...
__name(hexRound, "hexRound");
function hexesOnLine(a, b2) {
  const dist = hexDistance(a, b2);
  if (dist === 0)
    return [{ q: a.q, r: a.r }];
  const N3 = dist;
  const nudge = 1e-6;
...
__name(hexesOnLine, "hexesOnLine");

// ../games/capture-the-lobster/dist/los.js
...
function getVisibleHexes(position, radius, walls, allHexes) {
  ...
  for (const hex of candidates) {
    const key = hexToString(hex);
    if (!allHexes.has(key) || key === posKey)
      continue;
...
__name(getVisibleHexes, "getVisibleHexes");

// ../games/capture-the-lobster/dist/combat.js
var CLASS_VISION = {
  rogue: 4,
  knight: 2,
  mage: 3
...
__name(beats, "beats");
function resolveCombat(units, walls) {
  const kills = [];
  for (let i = 0; i < units.length; i++) {
    for (let j = i + 1; j < units.length; j++) {
      ...
      if (a.team === b2.team)
        continue;
      const dist = hexDistance(a.position, b2.position);
      if (dist > 1)
        continue;
      if (a.unitClass === b2.unitClass) {
        if (dist === 0) {
          kills.push({
            killerId: a.id,
            victimId: b2.id,
          ...
          kills.push({
            killerId: b2.id,
            victimId: a.id,
            reason: `mutual kill (same class ${b2.unitClass} on same hex)`
          });
        }
      } else {
        if (beats(a.unitClass, b2.unitClass)) {
          kills.push({
            killerId: a.id,
            victimId: b2.id,
            reason: `${a.unitClass} beats ${b2.unitClass} in melee`
          });
        }
        if (beats(b2.unitClass, a.unitClass)) {
          kills.push({
            killerId: b2.id,
            victimId: a.id,
            reason: `${b2.unitClass} beats ${a.unitClass} in melee`
          });
        }
      }
    }
  ...
  for (const mage of units) {
    if (mage.unitClass !== "mage")
      continue;
    for (const target of units) {
      if (target.team === mage.team)
        continue;
      if (target.unitClass !== "knight")
        continue;
...
// ../games/capture-the-lobster/dist/fog.js
function getUnitVision(unit, walls, allHexes) {
  const radius = CLASS_VISION[unit.unitClass];
  return getVisibleHexes(unit.position, radius, walls, allHexes);
}
__name(getUnitVision, "getUnitVision");
function buildVisibleState(viewer, allUnits, walls, tiles, flags) {
  const visibleKeys = getUnitVision(viewer, walls, new Set(tiles.keys()));
  const allFlags = [];
  for (const team of ["A", "B"]) {
    const f2 = flags[team];
    const arr = Array.isArray(f2) ? f2 : [f2];
    for (const flag of arr) {
      allFlags.push({ team, ...flag });
    }
  }
  const unitsByHex = /* @__PURE__ */ new Map();
  ...
  const flagsByHex = /* @__PURE__ */ new Map();
  for (const f2 of allFlags) {
    if (!f2.carried) {
      flagsByHex.set(hexToString(f2.position), f2.team);
    } else if (f2.carrierId) {
      const carrier = allUnits.find((u) => u.id === f2.carrierId && u.alive);
      if (carrier) {
        flagsByHex.set(hexToString(carrier.position), f2.team);
      }
    }
  }
  const result = [];
  for (const key of visibleKeys) {
    ...
    const tileType = tiles.get(key) ?? "ground";
    const tile = {
      q: hex.q,
      r: hex.r,
      type: tileType
    ...
    if (unitsHere && unitsHere.length > 0) {
      ...
      const isCarrying = allFlags.some((f2) => f2.carrierId === u.id && f2.carried);
      tile.unit = {
        ...isAlly ? { id: u.id } : {},
        team: u.team,
...
__name(buildVisibleState, "buildVisibleState");

// ../games/capture-the-lobster/dist/map.js
function getMapRadiusForTeamSize(teamSize) {
  const table = { 2: 5, 3: 6, 4: 7, 5: 8, 6: 9 };
  return table[teamSize] ?? Math.max(5, teamSize + 3);
}
__name(getMapRadiusForTeamSize, "getMapRadiusForTeamSize");
function getFlagCountForTeamSize(teamSize) {
  return teamSize >= 5 ? 2 : 1;
}
__name(getFlagCountForTeamSize, "getFlagCountForTeamSize");
...
function rotate180(hex) {
  return { q: -hex.q, r: -hex.r };
}
__name(rotate180, "rotate180");
function bfsReachable(start, passable, allKeys) {
...
__name(bfsReachable, "bfsReachable");
function generateMap(config) {
  ...
  const flagPositionsA = [{ q: 0, r: radius - 1 }];
  const flagPositionsB = [rotate180(flagPositionsA[0])];
  if (flagCount >= 2) {
    const rotated = { q: -(radius - 1), r: radius - 1 };
    flagPositionsA.push(rotated);
    flagPositionsB.push(rotate180(rotated));
  }
  const baseAKeys = /* @__PURE__ */ new Set();
  const baseBKeys = /* @__PURE__ */ new Set();
  function pickSpawns(flag, baseType, baseKeySet, count) {
    const neighbors = getNeighbors(flag).filter((n2) => tiles.has(hexToString(n2)));
  ...
  __name(pickSpawns, "pickSpawns");
  const basesA = [];
  const basesB = [];
  for (let i = 0; i < flagCount; i++) {
    tiles.set(hexToString(flagPositionsA[i]), "base_a");
    baseAKeys.add(hexToString(flagPositionsA[i]));
    for (const n2 of getNeighbors(flagPositionsA[i])) {
      const nk = hexToString(n2);
    ...
    for (const n2 of getNeighbors(flagPositionsB[i])) {
      const nk = hexToString(n2);
  ...
  for (const key of baseAKeys)
    protectedKeys.add(key);
  for (const key of baseBKeys)
    protectedKeys.add(key);
  for (const key of [...baseAKeys, ...baseBKeys]) {
    const hex = stringToHex(key);
    for (const n2 of getNeighbors(hex)) {
      const nk = hexToString(n2);
      if (tiles.has(nk))
        protectedKeys.add(nk);
    }
  }
  ...
  for (const seedKey of wallSeeds) {
    ...
    for (const n2 of neighbors) {
      if (grown >= growCount)
        break;
      const nk = hexToString(n2);
      const mirrorN = rotate180(n2);
      const mirrorNk = hexToString(mirrorN);
      if (tiles.has(nk) && tiles.has(mirrorNk) && !protectedKeys.has(nk) && !protectedKeys.has(mirrorNk) && !wallKeys.has(nk)) {
        wallKeys.add(nk);
  ...
  const borderHexes = hexesInRadius({ q: 0, r: 0 }, radius + 1).filter((h) => hexDistance(h, { q: 0, r: 0 }) === radius + 1);
  ...
  return {
    tiles,
    radius: radius + 1,
    // Include the border ring in the reported radius
    bases: { A: basesA, B: basesB }
  };
}
__name(generateMap, "generateMap");
function ensureConnectivity(tiles, allKeys, flagA, flagB, wallKeys, protectedKeys) {
  const flagAKey = hexToString(flagA);
  const flagBKey = hexToString(flagB);
  const isPassable = /* @__PURE__ */ __name((key) => tiles.get(key) !== "wall", "isPassable");
  let reachable = bfsReachable(flagAKey, isPassable, allKeys);
  if (reachable.has(flagBKey))
    return;
  const parent = /* @__PURE__ */ new Map();
  const visited = /* @__PURE__ */ new Set();
  const queue = [flagAKey];
  visited.add(flagAKey);
  while (queue.length > 0) {
    const current2 = queue.shift();
    if (current2 === flagBKey)
      break;
    const hex = stringToHex(current2);
    const neighbors = getNeighbors(hex);
    neighbors.sort((a, b2) => {
      const aWall = tiles.get(hexToString(a)) === "wall" ? 1 : 0;
      const bWall = tiles.get(hexToString(b2)) === "wall" ? 1 : 0;
      return aWall - bWall;
    });
    for (const n2 of neighbors) {
      const nk = hexToString(n2);
...
__name(ensureConnectivity, "ensureConnectivity");

// ../games/capture-the-lobster/dist/movement.js
var CLASS_SPEED = {
  rogue: 3,
  knight: 2,
  mage: 1
};
function validatePath(unit, path) {
...
function walkPath(start, path, validTiles) {
...
function resolveMovements(units, submissions, validTiles) {
  ...
  while (changed) {
    changed = false;
    const teamPositionMap = /* @__PURE__ */ new Map();
    ...
    for (const group of teamPositionMap.values()) {
      if (group.length <= 1)
        continue;
      group.sort((a, b2) => a.finalIndex - b2.finalIndex);
      for (let i = 1; i < group.length; i++) {
        const state = group[i];
        let backtracked = false;
        for (let idx = state.finalIndex - 1; idx >= 0; idx--) {
          const candidate = state.positions[idx];
          const occupied = states.some((other) => other !== state && other.unit.team === state.unit.team && hexEquals(other.positions[other.finalIndex], candidate));
          if (!occupied) {
...
__name(resolveMovements, "resolveMovements");

// ../games/capture-the-lobster/dist/game.js
function getTurnLimitForRadius(radius) {
  return 20 + radius * 2;
}
__name(getTurnLimitForRadius, "getTurnLimitForRadius");
var DEFAULT_CONFIG = {
  turnLimit: 30,
  turnTimerSeconds: 30,
  teamSize: 4
};
function computeTileSets(mapTiles) {
  const wallSet = /* @__PURE__ */ new Set();
  const validTiles = /* @__PURE__ */ new Set();
...
__name(computeTileSets, "computeTileSets");
function createGameState(map, players, config) {
  const resolvedConfig = { ...DEFAULT_CONFIG, ...config };
  const spawnIndexA = { current: 0 };
  const spawnIndexB = { current: 0 };
  const allSpawnsA = map.bases.A.flatMap((b2) => b2.spawns);
  const allSpawnsB = map.bases.B.flatMap((b2) => b2.spawns);
...
function validateMoveForPlayer(state, playerId, path) {
  if (state.phase !== "in_progress") {
    return { valid: false, error: "Game is not in progress" };
  }
  const unit = state.units.find((u) => u.id === playerId);
  if (!unit)
    return { valid: false, error: `Unknown agent: ${playerId}` };
  if (!unit.alive)
    return { valid: false, error: "Dead units cannot move" };
  const moveUnit = {
    id: unit.id,
    team: unit.team,
...
__name(validateMoveForPlayer, "validateMoveForPlayer");
function submitMove(state, playerId, path) {
...
__name(submitMove, "submitMove");
function allMovesSubmitted(state) {
  const submissions = new Map(state.moveSubmissions);
  const aliveUnits = state.units.filter((u) => u.alive);
  return aliveUnits.every((u) => submissions.has(u.id));
}
__name(allMovesSubmitted, "allMovesSubmitted");
function resolveTurn(state) {
  const currentTurn = state.turn;
  const { wallSet, validTiles } = computeTileSets(state.mapTiles);
  const submissions = new Map(state.moveSubmissions);
  const units = state.units.map((u) => ({ ...u, position: { ...u.position } }));
  const flags = {
    A: state.flags.A.map((f2) => ({ ...f2, position: { ...f2.position } })),
    B: state.flags.B.map((f2) => ({ ...f2, position: { ...f2.position } }))
  };
  ...
  for (const unit of units) {
    if (!unit.alive && unit.respawnTurn === currentTurn) {
      unit.alive = true;
      unit.respawnTurn = void 0;
    }
  }
  const unitPositionsBefore = /* @__PURE__ */ new Map();
  for (const unit of units) {
  ...
  for (const [id2, path] of submissions) {
    moves.set(id2, [...path]);
  }
  ...
  for (const result of moveResults) {
    ...
    if (unit.carryingFlag) {
      ...
      const carriedFlag = flags[enemyTeam].find((f2) => f2.carrierId === unit.id);
      if (carriedFlag) {
        carriedFlag.position = { ...result.to };
      }
    }
  }
  const combatUnits = units.filter((u) => u.alive).map((u) => ({
    id: u.id,
    team: u.team,
    unitClass: u.unitClass,
    position: { ...u.position }
  ...
  const allSpawnsA = mapBases.A.flatMap((b2) => b2.spawns);
  const allSpawnsB = mapBases.B.flatMap((b2) => b2.spawns);
  ...
  for (const deadId of combatResult.deaths) {
    ...
    if (unit.carryingFlag) {
      ...
      const enemyTeam = unit.team === "A" ? "B" : "A";
      const droppedFlag = flags[enemyTeam].find((f2) => f2.carrierId === unit.id);
      if (droppedFlag) {
        droppedFlag.carried = false;
        droppedFlag.carrierId = void 0;
  ...
  for (const unit of units) {
    if (!unit.alive || unit.carryingFlag)
      continue;
    const enemyTeam = unit.team === "A" ? "B" : "A";
  ...
  for (const unit of units) {
    ...
    if (atHome) {
      ...
      const enemyTeam = unit.team === "A" ? "B" : "A";
      const capturedFlag = flags[enemyTeam].find((f2) => f2.carrierId === unit.id);
      if (capturedFlag) {
        const baseIdx = flags[enemyTeam].indexOf(capturedFlag);
        capturedFlag.carried = false;
        capturedFlag.carrierId = void 0;
  ...
  const unitPositionsAfter = /* @__PURE__ */ new Map();
  for (const unit of units) {
...
__name(resolveTurn, "resolveTurn");
function getStateForAgent(state, agentId, submittedMoves) {
  ...
  const fogUnits = state.units.map((u) => ({
    id: u.id,
    team: u.team,
    unitClass: u.unitClass,
    position: u.position,
  ...
  const flagsForFog = {
    A: state.flags.A.map((f2) => ({
      position: f2.position,
      carried: f2.carried,
      carrierId: f2.carrierId
    })),
    B: state.flags.B.map((f2) => ({
      position: f2.position,
      carried: f2.carried,
      carrierId: f2.carrierId
    }))
  ...
  let yourFlagStatus = "at_base";
  for (const f2 of yourFlags) {
    if (f2.carried) {
  ...
  const moveSubmitted = submittedMoves ? submittedMoves.has(agentId) : new Map(state.moveSubmissions).has(agentId);
  return {
    turn: state.turn,
    phase: state.phase,
    yourUnit: {
...
__name(getStateForAgent, "getStateForAgent");
function isGameOver(state) {
  return state.phase === "finished";
}
__name(isGameOver, "isGameOver");

// ../games/capture-the-lobster/dist/phases/team-formation.js
var TOOLS = [
  {
    name: "propose_team",
...
var TeamFormationPhase = class {
  static {
    __name(this, "TeamFormationPhase");
  }
  id = "team-formation";
  name = "Team Formation";
  tools = TOOLS;
  ...
  // -------------------------------------------------------------------------
  handleJoin(state, player, allPlayers) {
    if (state.unassigned.includes(player.id) || state.teams.some((t) => t.members.includes(player.id))) {
      return { state };
    }
    const newState = {
      ...state,
  ...
  handleTimeout(state, players) {
    ...
    const freeAgents = players.map((p) => p.id).filter((id2) => !assigned.has(id2));
    let freeIdx = 0;
    ...
    const incomplete = teams.filter((t) => t.members.length < this.teamSize).sort((a, b2) => b2.members.length - a.members.length);
    for (let i = 0; i < incomplete.length; i++) {
      const target = incomplete[i];
      if (target.members.length >= this.teamSize)
        continue;
    ...
    const selectedPlayerIds = /* @__PURE__ */ new Set();
    for (const t of selectedTeams) {
      for (const m of t.members)
        selectedPlayerIds.add(m);
    }
    const groups = selectedTeams.map((t) => t.members.map((id2) => {
      const p = players.find((pp) => pp.id === id2);
      return p ?? { id: id2, handle: id2 };
    }));
    const removed = players.filter((p) => !selectedPlayerIds.has(p.id)).map((p) => ({ id: p.id, handle: p.handle }));
    return {
      groups,
      metadata: {
  ...
  // -------------------------------------------------------------------------
  handlePropose(state, playerId, payload, players) {
    ...
    if (fromTeam && !toTeam) {
      ...
      if (!team.invites.includes(targetId))
        team.invites.push(targetId);
      return this.maybeComplete(newState, players);
    }
    const teamId = `team_${++newState.teamCounter}`;
    newState.teams.push({ id: teamId, members: [playerId], invites: [targetId] });
    newState.unassigned = newState.unassigned.filter((id2) => id2 !== playerId);
    return this.maybeComplete(newState, players);
  }
  handleAccept(state, playerId, payload, players) {
    ...
    if (currentTeam) {
      currentTeam.members = currentTeam.members.filter((id2) => id2 !== playerId);
      if (currentTeam.members.length === 0) {
        newState.teams = newState.teams.filter((t) => t.id !== currentTeam.id);
      }
    }
    team.invites = team.invites.filter((id2) => id2 !== playerId);
    team.members.push(playerId);
    newState.unassigned = newState.unassigned.filter((id2) => id2 !== playerId);
    return this.maybeComplete(newState, players);
  }
  handleLeave(state, playerId, players) {
    const team = state.teams.find((t) => t.members.includes(playerId));
    if (!team) {
    ...
    targetTeam.members = targetTeam.members.filter((id2) => id2 !== playerId);
...
// ../games/capture-the-lobster/dist/phases/class-selection.js
var ClassSelectionPhase = class {
  static {
    __name(this, "ClassSelectionPhase");
  }
  id = "class-selection";
  ...
  handleAction(state, action, players) {
    ...
    const unitClass = payload?.unitClass;
    ...
    const allPicked = newState.playerIds.every((id2) => id2 in newState.classPicks);
    if (allPicked) {
      return {
        state: newState,
  ...
  handleTimeout(state, players) {
    const filled = { ...state.classPicks };
    let idx = 0;
    for (const id2 of state.playerIds) {
      if (!(id2 in filled)) {
        filled[id2] = this.validClasses[idx % this.validClasses.length];
        idx++;
      }
    }
    const filledState = {
  ...
  // Helpers
  // -------------------------------------------------------------------------
  buildResult(state, players) {
    const allPlayers = state.playerIds.map((id2) => players.find((p) => p.id === id2)).filter((p) => p != null);
    return {
      groups: [allPlayers],
      metadata: {
        classPicks: state.classPicks
...
// ../games/capture-the-lobster/dist/plugin.js
function buildCtlSpectatorView(state, prevState, context) {
  const map = { tiles: new Map(state.mapTiles), radius: state.mapRadius, bases: state.mapBases };
  ...
  for (const team of ["A", "B"]) {
    ...
    for (const f2 of teamFlags) {
      flagsByHex.set(`${f2.position.q},${f2.position.r}`, team);
    }
  }
  for (const [key, tileType] of map.tiles) {
    ...
    if (unitsHere && unitsHere.length > 0) {
      ...
      tile.unit = {
        ...
        carryingFlag: primary.carryingFlag || void 0,
        alive: primary.alive,
      ...
      if (unitsHere.length > 1) {
        tile.units = unitsHere.map((u) => ({
          ...
          team: u.team,
          unitClass: u.unitClass,
          carryingFlag: u.carryingFlag || void 0,
          alive: u.alive,
  ...
  const kills = (state.lastKills ?? []).map((k) => ({
    killerId: k.killerId,
    victimId: k.victimId,
    reason: k.reason
  }));
  function flagStatus(flagArr) {
    for (const f2 of flagArr) {
      if (f2.carried && f2.carrierId) {
        return { status: "carried", carrier: f2.carrierId };
      }
    }
  ...
  const allHexKeys = /* @__PURE__ */ new Set();
  for (const [key, tileType] of map.tiles) {
    allHexKeys.add(key);
    if (tileType === "wall")
      walls.add(key);
  }
  const visibleA = /* @__PURE__ */ new Set();
  const visibleB = /* @__PURE__ */ new Set();
  const visibleByUnit = {};
  for (const u of units) {
    if (!u.alive)
      continue;
    const unitVision = getUnitVision({ id: u.id, position: u.position, unitClass: u.unitClass, team: u.team, alive: u.alive }, walls, allHexKeys);
  ...
  const chatA = relayMessages.filter((m) => m.type === "messaging" && m.scope === "team" && units.some((u) => u.id === m.sender && u.team === "A")).map((m) => ({ from: m.sender, message: m.data?.body ?? "", turn: m.turn }));
  const chatB = relayMessages.filter((m) => m.type === "messaging" && m.scope === "team" && units.some((u) => u.id === m.sender && u.team === "B")).map((m) => ({ from: m.sender, message: m.data?.body ?? "", turn: m.turn }));
  return {
    turn,
...
__name(buildCtlSpectatorView, "buildCtlSpectatorView");
var CTL_GUIDE = `# Capture the Lobster \u2014 Game Rules
...
var CaptureTheLobsterPlugin = {
  ...
  createInitialState(config) {
    ...
    return createGameState(map, config.players.map((p) => ({
      id: p.id,
      team: p.team,
      unitClass: p.unitClass
    })), {
  ...
  applyAction(state, playerId, action) {
    if (action.type === "game_start") {
      const started = { ...state, phase: "in_progress" };
      return {
        state: started,
    ...
    if (action.type === "turn_timeout") {
      ...
      for (const unit of current.units) {
        ...
        if (unit.alive && !submissions.has(unit.id)) {
          const result = submitMove(current, unit.id, []);
    ...
    if (action.type === "move" && playerId !== null) {
      const result = submitMove(state, playerId, action.path);
      if (!result.success)
        return { state };
  ...
  buildSpectatorView(state, prevState, context) {
  ...
  guide: CTL_GUIDE,
  getPlayerStatus(state, playerId) {
    let status = "\n## Your Status\n";
...
`;
  ...
  getSummary(state) {
    return {
      ...
      winner: state.winner,
      teams: {
        A: state.units.filter((u) => u.team === "A").map((u) => u.id),
        B: state.units.filter((u) => u.team === "B").map((u) => u.id)
      }
    };
  },
  spectatorDelay: 2,
  getPlayersNeedingAction(state) {
    if (state.phase !== "in_progress")
      return [];
    const submitted = new Set(new Map(state.moveSubmissions).keys());
  ...
  computePayouts(outcome, playerIds) {
    ...
    const entryCost = 10;
    for (const id2 of playerIds) {
      const stats = outcome.playerStats.get(id2);
      if (!stats) {
        payouts.set(id2, 0);
        continue;
      }
      payouts.set(id2, stats.team === outcome.winner ? entryCost : -entryCost);
    }
    return payouts;
  },
  createConfig(players, seed, options) {
    const classes = ["rogue", "knight", "mage"];
    const teams = options?.teams;
    const classPicks = options?.classPicks;
    ...
    if (hasTeams) {
      ...
      const teamIdMap = {};
      uniqueTeamIds.forEach((id2, i) => {
        teamIdMap[id2] = i === 0 ? "A" : "B";
      });
      ctlPlayers = enrichedPlayers.map((p) => ({
        id: p.id,
        team: p.team ? teamIdMap[p.team] : "A",
        unitClass: p.role ?? classes[0]
      }));
    } else {
    ...
    const teamSize = options?.teamSize ?? Math.max(ctlPlayers.filter((p) => p.team === "A").length, ctlPlayers.filter((p) => p.team === "B").length);
...
// ../games/oathbreaker/dist/types.js
var DEFAULT_OATH_CONFIG = {
  maxRounds: 12,
  startingPoints: 100,
  minPledge: 5,
  maxPledgePct: 50,
...
// ../games/oathbreaker/dist/game.js
...
function hashSeed2(str, round) {
  let hash2 = 0;
  const s = `${str}:${round}`;
  for (let i = 0; i < s.length; i++) {
    hash2 = (hash2 << 5) - hash2 + s.charCodeAt(i) | 0;
  }
  return hash2;
}
__name(hashSeed2, "hashSeed");
function seededShuffle(arr, seed, round) {
  const out = [...arr];
  const rng = mulberry322(hashSeed2(seed, round));
...
function cooperationBonus(pledge, config) {
...
__name(cooperationBonus, "cooperationBonus");
function createInitialState(config) {
  const playerIds = config.playerIds;
  const players = playerIds.map((id2) => ({
    id: id2,
...
function validateAction(state, playerId, action) {
  ...
  if (action.type === "propose_pledge") {
    if (pairing.phase !== "pledging")
      return false;
    const opponent = state.players.find((p) => p.id === (isPlayer1 ? pairing.player2 : pairing.player1));
    if (!opponent)
      return false;
    const lowerBalance = Math.min(player.balance, opponent.balance);
    const maxPledge = lowerBalance * (state.config.maxPledgePct / 100);
    if (action.amount < state.config.minPledge)
      return false;
    if (action.amount > maxPledge + 1e-3)
      return false;
    return true;
  }
  if (action.type === "submit_decision") {
    ...
    if (existing !== null)
      return false;
    if (action.decision !== "C" && action.decision !== "D")
      return false;
    return true;
  }
  return false;
...
function applyAction(state, playerId, action) {
  ...
  if (action.type === "propose_pledge" && playerId) {
    ...
    if (pairing.proposal1 !== null && pairing.proposal2 !== null && Math.abs(pairing.proposal1 - pairing.proposal2) < 1e-3) {
...
function advanceOrFinish(resolvedState) {
...
__name(advanceOrFinish, "advanceOrFinish");
...
function applyTimeoutDefaults(state) {
  const pairings = state.pairings.map((p) => {
    ...
    if (pairing.decision1 === null)
      pairing.decision1 = "C";
    if (pairing.decision2 === null)
      pairing.decision2 = "C";
    pairing.phase = "decided";
    return pairing;
  });
...
__name(applyTimeoutDefaults, "applyTimeoutDefaults");
...
function findPairing(state, playerId) {
  return state.pairings.find((p) => p.player1 === playerId || p.player2 === playerId);
}
__name(findPairing, "findPairing");
function findPairingIndex(state, playerId) {
  return state.pairings.findIndex((p) => p.player1 === playerId || p.player2 === playerId);
}
__name(findPairingIndex, "findPairingIndex");
function dollarValue(balance, totalDollarsInvested, totalSupply) {
  if (totalSupply <= 0)
    return 0;
  return balance * (totalDollarsInvested / totalSupply);
}
__name(dollarValue, "dollarValue");
function dollarPerPoint(totalDollarsInvested, totalSupply) {
  if (totalSupply <= 0)
    return 0;
  return totalDollarsInvested / totalSupply;
}
__name(dollarPerPoint, "dollarPerPoint");
function getAgentView(state, playerId) {
  const player = state.players.find((p) => p.id === playerId);
  if (!player)
    return null;
  const pairing = findPairing(state, playerId);
...
__name(getAgentView, "getAgentView");
...
__name(getSpectatorView, "getSpectatorView");

// ../games/oathbreaker/dist/plugin.js
var OATHBREAKER_GUIDE = `# OATHBREAKER \u2014 Game Rules
...
- Points have real dollar value: your balance \xD7 (totalDollarsInvested / totalSupply)
...
**Cooperation bonus formula:** \`pledge \xD7 0.10 \xD7 ln(pledge/100 + 1)^0.75\`
...
\`dollarValue = balance \xD7 (totalDollarsInvested / totalSupply)\`
...
`;
  ...
  getPlayersNeedingAction(state) {
    ...
    for (const pairing of state.pairings) {
      if (pairing.phase === "decided")
        continue;
      if (pairing.phase === "pledging") {
        if (pairing.proposal1 === null)
          needed.push(pairing.player1);
        if (pairing.proposal2 === null)
          needed.push(pairing.player2);
  ...
  buildSpectatorView(state, _prevState, _context) {
    return getSpectatorView(state);
  },
  isOver(state) {
    return state.phase === "finished";
  ...
  getOutcome(state) {
    ...
    }).sort((a, b2) => b2.dollarValue - a.dollarValue);
    return {
      rankings,
      dollarPerPoint: dpp,
  ...
  computePayouts(outcome, playerIds) {
    const payouts = /* @__PURE__ */ new Map();
    for (const id2 of playerIds) {
      const ranking = outcome.rankings.find((r) => r.id === id2);
      payouts.set(id2, ranking ? ranking.dollarValue - 1 : 0);
    }
    return payouts;
  },
  createConfig(players, seed, options) {
...
// src/do/GameRoomDO.ts
var TAG_SPECTATOR = "spectator";
var GameRoomDO = class extends DurableObject {
  static {
    __name(this, "GameRoomDO");
  }
  ...
  _plugin = null;
  _state = null;
  _prevProgressState = null;
  // state at last progress point (spectator delay)
  _actionLog = [];
  _progress = { counter: 0, snapshots: [0] };
  _relay = [];
  // ─────────────────────────────────────────────────────────────────────────
  ...
  async alarm() {
    ...
    const deadline = await this.ctx.storage.get("deadline");
    if (!deadline) return;
  ...
  async handleCreate(request) {
    ...
    const { gameType: rawGameType, config, playerIds, handleMap, teamMap, gameId: bodyGameId } = body ?? {};
    ...
    const gameId = bodyGameId ?? (this.ctx.id.name ?? "");
    const meta = {
      gameId,
      gameType,
      playerIds,
    ...
    const progress = { counter: 0, snapshots: [0] };
    await Promise.all([
  ...
  async handleAction(request) {
    ...
    if (action === void 0) return Response.json({ error: "action is required" }, { status: 400 });
  ...
  async handleResult() {
    ...
    const leaves = this._actionLog.map((e, i) => ({
      actionIndex: i,
      playerId: e.playerId,
  ...
  async handleTool(request) {
    ...
    const msg = {
      ...
      scope: relayObj.scope ?? "all",
      pluginId: relayObj.pluginId,
      sender: playerId,
  ...
  resolveRelayRecipients(msg) {
    ...
    const recipients = /* @__PURE__ */ new Set([msg.sender]);
    if (recipientId) recipients.add(recipientId);
    return [...recipients];
  }
  broadcastRelayMessage(msg) {
  ...
  // Core game logic
  // ─────────────────────────────────────────────────────────────────────────
  async applyActionInternal(playerId, action) {
    ...
    if (result.deadline !== void 0) {
      if (result.deadline === null) {
        ...
        const deadlineMs = Date.now() + result.deadline.seconds * 1e3;
  ...
  writeSummaryToD1() {
    if (!this._meta || !this._plugin) return;
    const summary = typeof this._plugin.getSummary === "function" ? this._plugin.getSummary(this._state) : {};
  ...
  buildSpectatorMessage() {
    ...
    const view = delay > 0 && this._prevProgressState ? this._plugin.buildSpectatorView(this._prevProgressState, null, ctx) : this._plugin.buildSpectatorView(this._state, this._prevProgressState, ctx);
    return {
      type: "state_update",
      gameType: this._meta.gameType,
      handles: this._meta.handleMap,
  ...
  async ensureLoaded() {
    ...
    await this.ctx.blockConcurrencyWhile(async () => {
      ...
      const [meta, state, prevProgressState, actionLog, progress, relay] = await Promise.all([
        ...
        this.ctx.storage.get("prevProgressState"),
        this.ctx.storage.get("actionLog"),
        this.ctx.storage.get("progress"),
        this.ctx.storage.get("relay")
      ]);
      if (!meta) {
        this._loaded = true;
        return;
...
var TAG_SPECTATOR2 = "spectator";
var LobbyDO = class extends DurableObject2 {
  static {
    __name(this, "LobbyDO");
  }
  _loaded = false;
  ...
  async handleCreate(request) {
    ...
    const now = Date.now();
    const deadlineMs = noTimeout ? null : firstPhase.timeout != null ? now + firstPhase.timeout * 1e3 : null;
    this._meta = {
      lobbyId,
      gameType,
      currentPhaseIndex: 0,
  ...
  async handleJoin(request) {
    ...
    const agent = { id: playerId, handle, elo: elo ?? 1e3, joinedAt: Date.now() };
    ...
    if (phase.acceptsJoins && phase.handleJoin) {
      const agentInfo = { id: playerId, handle };
      try {
  ...
  async handleDisband() {
    ...
    this.broadcastUpdate();
    for (const ws of this.ctx.getWebSockets(TAG_SPECTATOR2)) {
      try {
        ws.close(1e3, "Lobby disbanded");
      } catch {
      }
    }
    return Response.json({ ok: true });
  ...
  handleWebSocket() {
    const pair = new WebSocketPair();
    const [client, server] = Object.values(pair);
    this.ctx.acceptWebSocket(server, [TAG_SPECTATOR2]);
    if (this._meta) {
  ...
  async advancePhase(phaseResult) {
    if (!this._meta) return;
    ...
    if (nextIndex < phases.length) {
      ...
      if (!this._meta.noTimeout && nextPhase.timeout != null) {
        const deadlineMs = Date.now() + nextPhase.timeout * 1e3;
  ...
  buildState(playerId) {
    ...
    const filteredRelay = playerId ? this._relay.filter((msg) => this.isRelayVisible(msg, playerId, currentPhase)) : this._relay;
    const state = {
  ...
  isRelayVisible(msg, playerId, currentPhase) {
    if (msg.scope === "all") return true;
  ...
  async ensureLoaded() {
    ...
    await this.ctx.blockConcurrencyWhile(async () => {
      ...
      const [meta, agents, phaseState, relay] = await Promise.all([
        this.ctx.storage.get("meta"),
        this.ctx.storage.get("agents"),
        this.ctx.storage.get("phaseState"),
        this.ctx.storage.get("relay")
      ]);
...
var crypto6 = anyGlobal.crypto || anyGlobal.msCrypto;
...
function randomBytes2(length) {
  assert(crypto6 != null, "platform does not support secure random numbers", "UNSUPPORTED_OPERATION", {
    operation: "randomBytes"
  });
  assertArgument(Number.isInteger(length) && length > 0 && length <= 1024, "invalid length", "length", length);
  const result = new Uint8Array(length);
  crypto6.getRandomValues(result);
  return result;
}
...
var Transaction = class _Transaction {
  ...
  toString() {
    const output2 = [];
    const add2 = /* @__PURE__ */ __name((key) => {
      let value = this[key];
      if (typeof value === "string") {
...
function buildWrappedFallback(contract) {
  ...
  const send = /* @__PURE__ */ __name(async function(overrides) {
...
function buildWrappedMethod(contract, key) {
  ...
  const send = /* @__PURE__ */ __name(async function(...args) {
    const runner = contract.runner;
    assert(canSend(runner), "contract runner does not support sending transactions", "UNSUPPORTED_OPERATION", { operation: "sendTransaction" });
...
function hexRandom(bytes2) {
  const arr = new Uint8Array(bytes2);
  crypto.getRandomValues(arr);
  return Array.from(arr, (b2) => b2.toString(16).padStart(2, "0")).join("");
}
__name(hexRandom, "hexRandom");
async function handleAuthChallenge(_request, env) {
...
__name(handleAuthChallenge, "handleAuthChallenge");
async function handleAuthVerify(request, env) {
  ...
  const { nonce: rawNonce, signature: rawSig, address: rawAddr, name: rawName } = body ?? {};
...
Nonce: ${nonce}`;
  ...
  try {
    recoveredAddress = ethers_exports.verifyMessage(message, signature);
  } catch (err) {
  ...
  if (env.RPC_URL && env.REGISTRY_ADDRESS && env.ERC8004_ADDRESS) {
    try {
      const provider = new ethers_exports.JsonRpcProvider(env.RPC_URL);
      const registryAbi = ["function nameToAgent(bytes32) view returns (uint256)"];
      const registry = new ethers_exports.Contract(env.REGISTRY_ADDRESS, registryAbi, provider);
      const nameKey = ethers_exports.keccak256(ethers_exports.toUtf8Bytes(name.toLowerCase()));
      ...
      const erc8004 = new ethers_exports.Contract(env.ERC8004_ADDRESS, erc8004Abi, provider);
...
// src/db/elo.ts
var D1EloTracker = class _D1EloTracker {
  constructor(db) {
    this.db = db;
  }
  static {
    __name(this, "D1EloTracker");
  }
  static fromEnv(env) {
    return new _D1EloTracker(env.DB);
  }
  // ---------------------------------------------------------------------------
  // Player reads
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  static calculateEloChange(teamElo, opponentElo, result, kFactor = 32) {
  ...
  // ---------------------------------------------------------------------------
  async recordGameResult(matchId, gameType, players) {
    ...
    const avgElo = /* @__PURE__ */ __name((group) => group.length === 0 ? 1200 : group.reduce((s, p) => s + p.db.elo, 0) / group.length, "avgElo");
    ...
    ).bind(matchId, gameType, null, 0, isAllDraw ? null : "W", now, now).run();
    for (const p of dbPlayers) {
      ...
      const delta = _D1EloTracker.calculateEloChange(myGroupElo, oppGroupElo, result);
      ...
      ).bind(matchId, p.db.id, result === "win" ? "W" : result === "loss" ? "L" : "D", "unknown", p.db.elo, newElo).run();
      await this.db.prepare(
        "UPDATE players SET elo = ?, games_played = games_played + 1, wins = wins + ? WHERE id = ?"
      ).bind(newElo, won, p.db.id).run();
    }
  }
  // ---------------------------------------------------------------------------
  // Match history
...
__name(rowToPlayer, "rowToPlayer");
...
async function handleListLobbies(env) {
  const rows = await env.DB.prepare(
    "SELECT id, game_type, team_size, phase, created_at, game_id FROM lobbies WHERE phase NOT IN ('failed', 'game') ORDER BY created_at DESC LIMIT 50"
  ).all();
...
__name(handleListLobbies, "handleListLobbies");
async function handleCreateLobby(request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
  const gameType = body?.gameType ?? "capture-the-lobster";
  ...
  try {
    ...
    ).bind(lobbyId, gameType, teamSize, "running", (/* @__PURE__ */ new Date()).toISOString()).run();
  } catch (err) {
...
__name(handleCreateLobby, "handleCreateLobby");
...
var _summariesTableReady = false;
async function ensureGameSummariesTable(env) {
...
__name(ensureGameSummariesTable, "ensureGameSummariesTable");
...
async function handlePlayerState(playerId, env) {
...
__name(handlePlayerState, "handlePlayerState");
async function handlePlayerMove(playerId, request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
...
__name(handlePlayerMove, "handlePlayerMove");
async function handlePlayerLobbyJoin(playerId, request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
  ...
  if (joinBody?.phase !== "game" && joinBody?.phase !== "starting") {
    ...
    ).bind(playerId, lobbyId, (/* @__PURE__ */ new Date()).toISOString()).run();
  }
  return Response.json(joinBody, { status: joinResp.status });
}
__name(handlePlayerLobbyJoin, "handlePlayerLobbyJoin");
async function handlePlayerLobbyAction(playerId, request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
  const lobbySession = await getPlayerLobbySession(playerId, env);
  if (!lobbySession) {
...
__name(handlePlayerLobbyAction, "handlePlayerLobbyAction");
async function handlePlayerLobbyTool(playerId, request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
  const lobbySession = await getPlayerLobbySession(playerId, env);
  if (!lobbySession) {
...
__name(handlePlayerLobbyTool, "handlePlayerLobbyTool");
async function handlePlayerTool(playerId, request, env) {
  const body = await parseJsonBody(request);
  if (body instanceof Response) return body;
  const { relay } = body ?? {};
  if (!relay?.type || !relay?.pluginId) {
...
__name(handlePlayerTool, "handlePlayerTool");
...
async function handleProfile(handle, env) {
  const tracker = D1EloTracker.fromEnv(env);
...
async function handlePlayerStats(playerId, env) {
  const tracker = D1EloTracker.fromEnv(env);
...
function getGameDO(env, gameId) {
  return env.GAME_ROOM.get(env.GAME_ROOM.idFromName(gameId));
}
__name(getGameDO, "getGameDO");
function getLobbyDO(env, lobbyId) {
  return env.LOBBY.get(env.LOBBY.idFromName(lobbyId));
}
__name(getLobbyDO, "getLobbyDO");
async function getPlayerGameSession(playerId, env) {
  return env.DB.prepare(
    "SELECT game_id, game_type FROM game_sessions WHERE player_id = ?"
  ).bind(playerId).first();
}
__name(getPlayerGameSession, "getPlayerGameSession");
async function getPlayerLobbySession(playerId, env) {
  return env.DB.prepare(
    "SELECT lobby_id FROM lobby_sessions WHERE player_id = ?"
  ).bind(playerId).first();
}
__name(getPlayerLobbySession, "getPlayerLobbySession");
function forwardToGameDO(env, gameId, subPath, request) {
  const stub = getGameDO(env, gameId);
  const url = new URL(request.url);
  url.pathname = subPath;
  return stub.fetch(new Request(url.toString(), request));
}
__name(forwardToGameDO, "forwardToGameDO");
function forwardToLobbyDO(env, lobbyId, subPath, request) {
  const stub = getLobbyDO(env, lobbyId);
  const url = new URL(request.url);
  url.pathname = subPath;
  return stub.fetch(new Request(url.toString(), request));
}
__name(forwardToLobbyDO, "forwardToLobbyDO");
...
// .wrangler/tmp/bundle-zAZCke/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-zAZCke/middleware-loader.entry.ts
