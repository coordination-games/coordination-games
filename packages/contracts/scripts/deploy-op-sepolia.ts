...
async function main() {
  ...
  const registry = await CoordinationRegistry.deploy(erc8004Addr, usdcAddr, creditsAddr, treasury);
  ...
  if (actualRegistryAddr !== registryAddr) {
    throw new Error(
      `Registry address mismatch: expected ${registryAddr}, got ${actualRegistryAddr}`,
    );
  }
  ...
  if (actualGameAnchorAddr !== gameAnchorAddr) {
    throw new Error(
      `GameAnchor address mismatch: expected ${gameAnchorAddr}, got ${actualGameAnchorAddr}`,
    );
  }
