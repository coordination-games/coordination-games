...
export class GameClient {
	...
	async callPluginTool(
		...
		const raw = await this.api.post("/api/player/tool", {
			relay: { type: toolName, pluginId, data: args, scope: "all" },
		});
		return this.processResponse(raw);
	}
	...
	async lobbyTool(pluginId: string, tool: string, args: any): Promise<any> {
		await this.ensureAuth();
		return this.api.post("/api/player/lobby/tool", {
			relay: { type: tool, pluginId, data: args, scope: "all" },
		});
	}
	...
	private processResponse(raw: any): any {
		...
		const hasRelay =
			Array.isArray(raw.relayMessages) && raw.relayMessages.length > 0;
		if (!hasRelay) {
		...
		const { relayMessages: _r, ...rest } = raw;
		return { ...rest, messages: output.messages, reasoning: output.reasoning };
	}
}
