...
- [x] **Refactor `claude-bot.ts`** — Game-generic, uses Agent SDK `createSdkMcpServer()` + `tool()` with in-process MCP backed by GameClient (REST + pipeline). Uses `get_guide()` for game rules instead of hardcoded prompts.
- [x] **Update `lobby-runner.ts`** — Bots get `createBotToken()` tokens, create GameClient instances, use Agent SDK query() with in-process MCP for lobby and pre-game phases.
- [x] **Plugin tools for bots** — `createBotMcpServer()` accepts plugins, registers `mcpExpose: true` tools dynamically (e.g. chat from BasicChatPlugin).
- [x] **GameClient shared code** — Bots use same GameClient + pipeline as CLI players. REST API is the single path.
